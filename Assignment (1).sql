...
SELECT `Incident Date`, `Call Number`
FROM fireIncidents
...
SELECT `Incident date`,`Call Number`,`Station Area`
FROM fireIncidents 
...
SELECT `Ignition Cause`,COUNT( `Call Number`) FROM fireIncidents
...
SELECT COUNT(fireIncidents.`Incident Number`)
FROM fireCalls 
INNER JOIN fireIncidents ON 
firecalls.`Battalion`=fireIncidents.`Battalion`;
